...
var Kalendae = function (targetElement, options) {
	...
	var self = this,
		...
		/*
			frog Modification
			-----------------------------
			Add a notch to the container.
		*/
		$notch = util.make('div', {'class':classes.notch}, $container),
	...
	/*
		frog Modification
		------------------------------------
		Allow notable dates to be specified.
	*/
	if (typeof opts.notable === 'function') {
		self.notable = opts.notable;
	} else if (!!opts.notable) {
		var ndates = parseDates(opts.notable, opts.parseSplitDelimiter);
		self.notable = function (input) {
			input = moment(input).yearDay();
			if (input < 1 || !self._sel || self._sel.length < 1) return false;
			var i = ndates.length;
			while (i--) if (ndates[i].yearDay() === input) return true;
			return false;
		}
	} else {
		self.notable = function () {return false;}
	}
...
Kalendae.prototype = {
	defaults : {
		...
		/*
			...
			Allow notable dates to be specified.
		*/
		notable:				null,			/* array of dates, or function to be passed a date */
		...
		columnHeaderLength:		1,				/* number of characters to show in the column headers */
		titleFormat:			'YYYY, MMMM',	/* format mask for month titles. See momentjs.com for rules */
	...
	classes : {
		...
		*/
		...
		/*
			frog Modification
			------------------------------------------
			Specify a class for the year in the title.
		*/
		titleYear		:'k-title-year',
		...
		/*
			frog Modification
			----------------------------------
			Specify a class for notable dates.
		*/
		dayNotable		:'k-notable',
	...
	draw : function draw() {
		...
		do {
			...
			/*
				frog Modification
				-----------------------------------------------------------------------
				For formatting purposes, we need the wrap the year in it's own element.
			*/
			cal.caption.innerHTML = month.format(this.settings.titleFormat).replace(/([0-9]{4}), /,"<span class=\"" + classes.titleYear + "\">$1</span>");
